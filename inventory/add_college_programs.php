...
<body>
                    ...
                    <div class="form-holder">
                        <div class="form-content">
                            <div class="form-items">
                                <h4>Books Programs</h4>
                                <p><strong>Important:</strong> To maintain precise records, kindly complete the fields related to the book programs. Include the necessary details corresponding to the books in your inventory. Your cooperation is highly appreciated.</p>
                                <form action="inventorycommands.php" method="POST" id="addcollegeprogramsForm" onsubmit="return validateCheckbox()">
                                    <div class="col-md-12">
                                        <strong><label for="collegeID"></label></strong>
                                        <strong><label for="lastInsertedID" id="lastInsertedIDLabel">BOOK ID: <?php echo htmlspecialchars($lastInsertedID); ?></label></strong>
                                        <input type="hidden" class="form-control" name="collegeID" id="collegeID" value="<?php echo htmlspecialchars($lastInsertedID); ?>">
                                    </div>
                                    <div class="col-md-12">
                                        <strong><label for="collegeID">Select Programs(s)</label></strong>
                                        <?php
                                        $sql = "SELECT program_id, program_name FROM tbl_programs";
                                        $result = mysqli_query($db_connection, $sql);

                                        while ($row = mysqli_fetch_assoc($result)) {
                                            echo '<div class="form-check">';
                                            echo '<input type="checkbox" class="form-check-input" id="program_' . $row["program_id"] . '" name="programs[]" value="' . $row["program_id"] . '">';
                                            echo '<label class="form-check-label" for="program_' . $row["program_id"] . '">' . $row["program_name"] . '</label>';
                                            echo '</div>';
                                        }
                                        ?>
                                    </div>
                                    ...
                                    <div class="form-button mt-3">
                                        <button type="submit" class="btn btn-primary mt-2" name="addBookCollegePrograms" id="addsubmitBookCollegePrograms">&plus; Book Programs</button>
                                    </div>
                                </form>
                            </div>
                        </div>
    ...
    <script>
        ...
        function validateCheckbox() {
            var checkboxes = document.querySelectorAll('input[name="programs[]"]:checked');
            if (checkboxes.length === 0) {
                // Use SweetAlert for a more user-friendly alert
                Swal.fire({
                    icon: 'warning',
                    title: 'Oops...',
                    text: 'Please select at programs related to the books.',
                });
