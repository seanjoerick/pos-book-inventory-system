...
?>
    ...
    <main>
		...
		<div id="existingStudentSection" style="display:none;">
			...
			<select name="existingStudentNumber" id="existingStudentNumber" style="margin-bottom:5px; margin-left: 176px; position: relative; top: -5px; width: 400px;" size="4">
				...
				while ($row = mysqli_fetch_assoc($result)) {
					echo '<option value="' . $row['customer_id'] . '">' . $row['last_name'] . ', ' . $row['first_name'] . '(' . $row['student_number'] . ')</option>';
				}
				?>
			</select>

			<!-- PICK TEACHER -->
			<label for="advisoryTeacher" style="position: relative; top: -115px; left: -195px;">Select Advisory Teacher(if required):</label>
			<input type="text" id="teacherSearchInput" oninput="filterTeacherOptions()" placeholder="Search..." style="margin-bottom: 5px; position: relative; top: -115px; left: -195px;">
			<select name="advisoryTeacher" id="advisoryTeacher" size="4" style="position: relative; top: -10px; left: -405px; width: 205px;">
					<option disabled selected value="na">Select</option>
