...
$searchPublicationYear = $_POST['searchPublicationYear'];
$searchSubjectCode = $_POST['searchSubjectCode'];
$searchYearLevel = $_POST['searchYearLevel'];

// Modify your query based on the search parameters
$query = "SELECT DISTINCT b.*, y.year_level_name, s.subject_code 
    FROM tbl_books b
    INNER JOIN tbl_yearlevels y ON b.year_level_id = y.year_level_id
    INNER JOIN tbl_subjects s ON b.subject_id = s.subject_id
    WHERE b.status = 'active' AND b.quantity_available > 0 AND
    (y.year_level_type LIKE '%$searchType%' OR '$searchType' = 'all') AND
    (b.title LIKE '%$searchTitle%') AND
    (b.author LIKE '%$searchAuthor%') AND
    (b.publication_year LIKE '%$searchPublicationYear%') AND
    (s.subject_id LIKE '%$searchSubjectCode%' OR '$searchSubjectCode' = 'all') AND
    (y.year_level_id = '$searchYearLevel' OR '$searchYearLevel' = 'all')";
